...
export class InMemoryKeyValueStorage extends KeyValueStorage {
  ...
  async getKey(key: string, refresh: boolean = true) {
    ...
    if (
      this.keyValueMap[key].ttl &&
      this.keyValueMap[key].time + this.keyValueMap[key].ttl < Date.now() / 1000
    ) {
      delete this.keyValueMap[key];
      return null;
    }

    if (refresh) this.keyValueMap[key].time = Date.now();

    return this.keyValueMap[key].value;
  }

  async saveKey(key: string, value: any, ttl: number | null | undefined = 3600) {
